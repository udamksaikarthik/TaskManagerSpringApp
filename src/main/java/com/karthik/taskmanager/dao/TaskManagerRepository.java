...
public interface TaskManagerRepository extends JpaRepository<Task, Integer>{
	...
	@Modifying
    @Transactional
    @Query("DELETE FROM Task t WHERE TRIM(t.taskName) = :selectedTaskName AND TRIM(t.username) = :username")
    void deleteTaskByTaskName(String selectedTaskName, String username);
